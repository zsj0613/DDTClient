 ...
 * @author iiley
 */
public class RoboSearcher implements IPathSearcher {
	...
	public function RoboSearcher(step:Number, maxDistance:Number, num:Number=4){
		this.step = step;
	 ...
	 * 返回查找到的路径,null表示查找失败
	 */
	private function searchWithWish(from : Point, tto : Point, tester : IHitTester, wish:Number, nodes:Array) : Boolean {
	 ...
	 * 如果返回null表示查找失败
	 */
	private function findFarestBlankPoint(from:Point, tto:Point, t:IHitTester):Point{
	 ...
	 */
	private function findReversseNearestBlankPoint(from:Point, tto:Point, t:IHitTester):Point{
	 ...
	 * 这里的起点和终点都必须保证不在障碍内
	 */
	private function doSearchWithWish(from : Point, tto : Point, tester : IHitTester, wish:Number, nodes:Array) : Boolean 
	{
		nodes.push(from);
