package ddt.hotSpring.view.frame
{
	...
	public class RoomInviteListItemView extends InviteItemBgAsset
	{
		...
		public function RoomInviteListItemView(info:Object)
		{
			_info = info;
			super();
			init();
		...
		private function __mouseClick(evt:MouseEvent):void
		{
			...
			if(_info is ConsortiaPlayerInfo)
			{
				SocketManager.Instance.out.sendHotSpringRoomInvite(_info.info.ID);
			}
			else
			{
				SocketManager.Instance.out.sendHotSpringRoomInvite(_info.ID);
			}
